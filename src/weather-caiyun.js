...
class CaiyunWeatherService {
	...
	formatCaiyunData(data) {
		try {
			...
			const currentUVIndex = realtimeLifeIndex.ultraviolet
				? realtimeLifeIndex.ultraviolet.desc
				: '未知';
			const currentComfort = realtimeLifeIndex.comfort ? realtimeLifeIndex.comfort.desc : '未知';
			...
			// UV指数
			const uvIndex =
				daily.ultraviolet && daily.ultraviolet[0] ? Math.round(daily.ultraviolet[0].max) : '未知';

			// 生活指数（今日）
			const lifeIndex = daily.life_index || {};
			const todayUVIndex =
				lifeIndex.ultraviolet && lifeIndex.ultraviolet[0] ? lifeIndex.ultraviolet[0].desc : '未知';
			const todayColdRisk =
				lifeIndex.coldRisk && lifeIndex.coldRisk[0] ? lifeIndex.coldRisk[0].desc : '未知';
			const todayDressing =
				lifeIndex.dressing && lifeIndex.dressing[0] ? lifeIndex.dressing[0].desc : '未知';
			const todayCarWashing =
				lifeIndex.carWashing && lifeIndex.carWashing[0] ? lifeIndex.carWashing[0].desc : '未知';
	...
	formatWeatherForEmail(weatherData) {
		...
		if (airQuality && !airQuality.error) {
			...
			weatherEmail.push(
				`🌫️ 空气质量: ${aqiLevel.emoji} ${aqiLevel.level} (AQI: ${airQuality.aqi})`
			);
		...
		// 使用自己计算的穿衣建议
		lifeAdvice.push(
			`👕 穿衣建议: ${dressingAdvice.emoji} ${dressingAdvice.level} - ${dressingAdvice.advice}`
		);
		...
		if (lifeAdvice.length > 0) {
			weatherEmail.push('', '💡 生活建议', '- - - - - - - - - - - - - - - -', ...lifeAdvice);
		}

		// 添加12小时天气预报
		const hourlyForecast = this.format12HourForecast(weatherData);
		if (hourlyForecast) {
			weatherEmail.push('', '⏰ 12小时预报', '- - - - - - - - - - - - - - - -', hourlyForecast);
		}
